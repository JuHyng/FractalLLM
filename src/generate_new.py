...
def verify_forward(
    ...
    for i, slot_data in enumerate(slots):        
        ...
        for pos in range(total_checked):
            pos = pos + slot_data["current_input_ids"].shape[1] - total_checked - 1
            ...
            if self.args.use_cache:
                if past_key_values.get_seq_length() > 0:
                    ...
                    for i in range(len(cache_batch_split)):
                        ...
                        active_slots[i]["current_input_ids"] = active_slots[i]["input_ids"][:, (cache_len):] # 1(last token of prefix) + draft_tokens (draft_len+1)
